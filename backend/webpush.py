...
application_server_key = b64urlencode(
    Vapid.from_file("private_key.pem").public_key.public_bytes(
...
def register_recv_hook(exchanger: vcc.RpcExchanger):
    async def handler(msg: vcc.RedisMessage):
        user_list: list[tuple[int, str]] = await exchanger.rpc.chat.get_users(id=msg["chat"])
        chat_name: str = await exchanger.rpc.chat.get_name(id=msg["chat"])
        send_nickname = None
        ...
        for id, nickname in user_list:
            if id not in _push_list:
                continue
            push_func = lambda: pywebpush.webpush(_push_list[id], json.dumps({
                "chatName": chat_name,
            ...
            task_list.append(asyncio.create_task(asyncio.get_running_loop().run_in_executor(None, push_func)))
