...
def oauthGhHttp(port:int=1234,table:dict[str,asyncio.Future]={}):
    ...
    async def callback_url(request):
        ...
        async with ClientSession(headers={"Accept":"application/json"}) as client:
            ...
            access=await client.post(GH_ACCESS_URL_TEMPLATE.format(**{"clientid":GH_CLIENTID,"clientsec":GH_CLIENTSEC,"callbackurl":GH_CALLBACKURL,"requestid":requestid,"code":code}))
            accesstoken=(await access.json())['access_token']
            userinfo=await client.get(GH_GET_USER,headers={"Authorization":"token "+accesstoken})
    ...
    return type("oauthGhHttp",(),{"__call__":lambda _:aiohttp.web._run_app(app,port=port),"app":app})()
